...
[Mouse Phenome
Database](http://phenome.jax.org/db/q?rtn=projects/projdet&reqprojid=289),
but the file is nearly 500 MB. We've extractly the overall intensities
...
`yint` are like `JAXW202-3168`, which those in the `do` object are
...
intensity), which the males are above and to the left (low X
...
It's also useful to look at the frequency of heterozygote genotypes on
the X chromosome. Particularly informative is a scatterplot of the
overall proportion of missing genotypes versus the proportion of
heterozygote calls on the X chromosome.
...
iplot(pmis, phetX, indID=ind_ids_geno(do), group=do$covar$Sex,
      chartOpts=list(xlab="Proportion missing genotypes",
                     ylab="Proportion het on X chr"))
```

There are a number of males with a high proportion of heterozygous
genotype calls on the X chromosome, but it looks like they all have
high rates of missing genotypes and so are likely bad DNA samples. The
proportion of heterozygous calls on the X chromosome in females covers
a very broad range, from about 5% to nearly 75%. And while this
feature can be useful for identifying females that were misrecorded as
males, it is not so informative for identifying males that were
misrecorded as females.
...
```{r males_high_het}
sort(phetX[sex=="M" & pmis < 0.05], decreasing=TRUE)[1:5]
```
...
```{r females_low_het}
sort(phetX[sex=="F" & pmis < 0.05])[1:6]
```

The lowest five include the four samples identified above, based on
array intensities, plus also `JAXW202.2490`, with heterozygous
frequency of `r round(phetX["JAXW202.2490"]*100)`%, but this might
just be real.
